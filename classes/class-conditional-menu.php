...
class Conditional_Menu {
	...
	public function register_hooks(): void {
		\add_filter( 'render_block', array( $this, 'filter_conditional_block' ), 10, 2 );
	}
	 ...
	 */
	public function filter_conditional_block( string $block_content, array $block ): string {
		...
		foreach ( $class_list as $class ) {
			if ( \str_starts_with( $class, self::ROLE_PREFIX ) ) {
				...
				if ( ! $this->current_user_has_role( $required_role ) ) {
					return '';
				}
				break;
			}
	 ...
	 * @return bool
	 */
	private function current_user_has_role( string $role ): bool {
